...
def _session_cleanup():
    ...
    expired_sessions = []
    for session_id in session_db:
        if (session_db[session_id]['active'] == False or
            (datetime.now() - datetime.strptime(
                session_db[session_id]['established'],
                '%Y-%m-%dT%H:%M:%SZ')) > timedelta(minutes=5)):
            expired_sessions.append(session_id)
    for session_id in expired_sessions:
        app.logger.debug("Deleting session %s" % session_id)
...
def login_post():
    ...
    If credentials are good, all stateful logins are processed in another thread
    and session is created in session_db with -
    - Username & privilege level copied from users DB
    - IP of user
    - Creation time
    """
    cleanup_thread = Thread(target=_session_cleanup,
                            args=[])
    ...
    if not ('username' in list(args) and 'password' in list(args)):
        app.logger.error(
            "Missing Username or PW for login attempt, got %s" % (args))
        return make_response(json.dumps({"reason" : "MissingUserorPW"}),
                             401)
    ...
    app.logger.info("Login request for user %s from %s" %
    ...
    if login_result:
        ...
        app.logger.info("Session created in for user %s session %s IP %s" %
                        (username, session.id, remote_address))

        def async_logins():
            """add functions that need stateful login here"""
        async_login_thread = Thread(target=async_logins,
                                    args=[session, session_db])
        async_login_thread.start()

    else:
        app.logger.error("Bad Username or PW for user:%s" % (username))
        return make_response(json.dumps({"reason" : "BadUserorPW"}),
                             401)
...
def logout_post():
    ...
    if current_session == "":
        return make_response(json.dumps({"reason" : "BadOrInactiveSession"}),
                             401)

from couchdb.mapping import Document, TextField, IntegerField, BooleanField, ListField, DictField, Mapping
import json, urllib2
...
def _get_pin(pin_num, driver_info):
    return _send_cmd(pin_num,driver_info)

def _set_pin(pin_num, key, value, driver_info):
    value = bool_or_string(value)
    pin_status = _get_pin(pin_num,driver_info)
    
    if pin_status.get('digital'):
        if key == 'output' and value in [True,False]:
            return _set_pin_output(pin_num,value, driver_info)
    
        elif key == 'status' and value in [True,False] and pin_status.get('output'):
...
def _get_shell_commands(pin_num, driver_info):
    shell_commands = {}
    shell_commands['True'] = "wget -O /dev/null %s%s/%s" % (driver_info.get('address'),pin_num,"HIGH")
    shell_commands['False'] = "wget -O /dev/null %s%s/%s" % (driver_info.get('address'),pin_num,"LOW")
...
class SpokeRestDuinoDriver(SpokeDriver):
    ...
    def get_pin(self, spoke, pin_num):
        return _get_pin(pin_num,_parse_driver_info(spoke))
    ...
    def set_pin(self, spoke, pin_num, key, value):
        return _set_pin(pin_num, key, value,_parse_driver_info(spoke))
    ...
    def get_shell_commands(self, spoke, pin_num):
        return _get_shell_commands(pin_num, _parse_driver_info(spoke))
